...
//    @Autowired
    ...
    @Autowired
    ControladorHistoria(ServicioDeArchivos servicioDeArchivos, RepositorioHistorias repositorioHistorias, RepositorioPacientes repositorioPacientes){
...
//
//                String url = MvcUriComponentsBuilder.fromMethodName(ControladorHistoria.class, "obtenerArchivo", archivo.getFileName().toString()).build().toString();
//
...
//    }
    ...
    @ResponseBody
    public ResponseEntity<Resource> obtenerArchivo(@PathVariable String archivoBuscado) {
        try {
    ...
    @PostMapping()
    @Transactional
    public ResponseEntity<Historia> guardarHistoria(@RequestParam(required = true) int idPaciente, @RequestParam("archivo") MultipartFile archivo) {
        try {
            ...
            String url = MvcUriComponentsBuilder.fromMethodName(ControladorHistoria.class, "obtenerArchivo", archivo.getOriginalFilename().replaceAll("\\s+", "_")).build().toString();
    ...
    public ResponseEntity<Historia> historiaModificada(@RequestParam int idHistoria, @RequestBody MultipartFile nuevoArchivo){
        try{
            ...
            servicioDeArchivos.guardar(nuevoArchivo);
            String url = MvcUriComponentsBuilder.fromMethodName(ControladorHistoria.class, "obtenerArchivo", nuevoArchivo.getOriginalFilename().replaceAll("\\s+", "_")).build().toString();
    ...
    public ResponseEntity<String> borrarHistoria(@RequestParam int idHistoria){
        try {
            ...
            String archivo = obtenerArchivo(String.valueOf(Paths.get(historiaABorrar.getDocumentoHistoria()))).getBody().getFilename();
